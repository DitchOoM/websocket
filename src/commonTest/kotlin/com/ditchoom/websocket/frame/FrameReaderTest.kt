...
class FrameReaderTest {
    ...
    @Test
    fun `incremental parsing - returns null when no data`() =
        runTest {
            val processor = StreamProcessor.builder(pool).buildSuspending()
            val reader = FrameReader(processor)
    ...
    @Test
    fun `incremental parsing - returns null with only 1 byte`() =
        runTest {
            val processor = StreamProcessor.builder(pool).buildSuspending()
    ...
    @Test
    fun `incremental parsing - returns null when header complete but payload missing`() =
        runTest {
    ...
    @Test
    fun `incremental parsing - returns null when extended length incomplete`() =
        runTest {
    ...
    @Test
    fun `incremental parsing - completes when more data arrives`() =
        runTest {
            val processor = StreamProcessor.builder(pool).buildSuspending()
            val reader = FrameReader(processor)
    ...
    @Test
    fun `error - 64-bit length with MSB set throws exception`() =
        runTest {
