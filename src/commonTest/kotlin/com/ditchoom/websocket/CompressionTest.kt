...
class CompressionTest {
    ...
    @Test
    fun inflateDeflateWSMessage() = runTestNoTimeSkipping {
        val uncompressedBuffer = (" Here's a repeating sentence that repeats words like repeat, " +
                "repeatedly.".repeat(40)).toReadBuffer()
    ...
    fun inflateExample7_2_3_1_Unfragmented() = runTest(timeout = 2.seconds) {
        val expectedBytes = byteArrayOf(
            0xc1.toByte(), 0x07, 0xf2.toByte(), 0x48, 0xcd.toByte(), 0xc9.toByte(),
            0xc9.toByte(), 0x07, 0x00, 0x00, 0x00, 0xff.toByte(), 0xff.toByte())
        val expectedBuffer = PlatformBuffer.wrap(expectedBytes)
        val ws = DefaultWebSocketClient(
            WebSocketConnectionOptions("", 3), pool, null)
    ...
    @Test // https://datatracker.ietf.org/doc/html/rfc7692#section-7.2.3.1
    fun inflateExample7_2_3_1_Fragmented() = runTestNoTimeSkipping(3) {
        val expectedBytes = byteArrayOf(
            0x41, 0x03, 0xf2.toByte(), 0x48, 0xcd.toByte(), // first frame
            0x80.toByte(), 0x04, 0xc9.toByte(), 0xc9.toByte(), 0x07, 0x00 // second frame
        )
        val expectedBuffer = PlatformBuffer.wrap(expectedBytes)
        val ws = DefaultWebSocketClient(
            WebSocketConnectionOptions("", 3), pool, null)
    ...
    fun inflateExample7_2_3_3() = runTestNoTimeSkipping {
        val expectedBytes = byteArrayOf(
            0xc1.toByte(), 0x0b, 0x00, 0x05, 0x00,
            0xfa.toByte(), 0xff.toByte(), 0x48, 0x65, 0x6c, 0x6c, 0x6f, 0x00
        )
        val expectedBuffer = PlatformBuffer.wrap(expectedBytes)
        val ws = DefaultWebSocketClient(
            WebSocketConnectionOptions("", 3), pool, null)
    ...
    fun inflateExample7_2_3_4() = runTestNoTimeSkipping {
        val expectedBytes = byteArrayOf(
            0xf3.toByte(), 0x48, 0xcd.toByte(), 0xc9.toByte(), 0xc9.toByte(), 0x07, 0x00, 0x00
        )
        val expectedBuffer = PlatformBuffer.wrap(expectedBytes)
    ...
    fun inflateExample7_2_3_5() = runTestNoTimeSkipping {
        val expectedBytes = byteArrayOf(
            0xf2.toByte(), 0x48, 0x05, 0x00, 0x00, 0x00, 0xff.toByte(), 0xff.toByte(),
            0xca.toByte(), 0xc9.toByte(), 0xc9.toByte(), 0x07, 0x00
        )
        val expectedBuffer = PlatformBuffer.wrap(expectedBytes)
        val decompressed = expectedBuffer.decompressWebsocketBuffer()
        assertEquals("Hello", decompressed.readString(decompressed.remaining()))
    ...
    @Test
    fun largeBuffer() = runTestNoTimeSkipping {
    ...
    private fun assertContentEquals(expected: ReadBuffer, actual: ReadBuffer) {
